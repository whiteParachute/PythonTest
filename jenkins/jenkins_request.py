            ...
            if not my_job.is_queued_or_running():
                ...
                while True:
                    if not my_job.is_queued_or_running():
                        ...
                        log.info(" " + str(start_time) + " 发起的" +
                                 my_job_name + "构建已经完成，构建的开始时间为" +
                                 start_time + ", 状态为" + status)
                        p2 = re.compile(r".*ERROR.*")
                        err_list = p2.findall(console_out)
                        log_info("打包日志为：" + str(console_out))
                        if status == "SUCCESS":
                            if len(change) > 0:
                                for data in change:
                                    for file_list in data['affectedPaths']:
                                        log.info(" 发起的" + my_job_name + " 变更的类："
                                                 + file_list)
                                    log.info(
                                        " 发起的" + my_job_name + " 变更的备注：" + data[
                                            "msg"])
                                    log.info(
                                        " 发起的" + my_job_name + " 变更的提交人：" +
                                        data[
                                            "author"]["fullName"])
                            else:
                            ...
                            if len(err_list) > 0:
                                log.warning(
                                    " 构建的" + my_job_name + "构建状态为成功，但包含以下错误：")
